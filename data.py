...
class MNISTSet(torch.utils.data.Dataset):
    def __init__(self, threshold=0.0, train=True, root="data/MNISTSet", full=False):
        self.train = train
        self.root = root
    ...
    def __getitem__(self, item):
        ...
        x_0 = torch.randn_like(s)
        C = torch.sum((x_0[:,None,:] - s[None,:,:])**2, axis=-1)
        row, col = linear_sum_assignment(C.cpu())
        col = torch.as_tensor(col, dtype=torch.long)
        x_1 = s[col] 

        # # pad to fixed size
        padding_size = self.max - x_1.size(0)
        x_0 = torch.cat([x_0, torch.zeros(padding_size, 2)], dim=0)
        x_1 = torch.cat([x_1, torch.zeros(padding_size, 2)], dim=0)

        mask = torch.zeros(self.max)
        mask[:c].fill_(1)

        return x_0, x_1, mask.bool()
...
class ShapeNet15kPointClouds(Uniform15KPC):
    def __init__(self, root="data/ShapeNetCore.v2.PC15k",
...
def collate_fn(batch):
    ...
    x_0 = torch.randn_like(s)
    # # C = torch.sum((x_0[:,None,:] - s[None,:,:])**2, axis=-1)
    # C = torch.cdist(x_0, s, p=2).pow(2)
    # row, col = linear_sum_assignment(C.cpu())
    # col = torch.as_tensor(col, dtype=torch.long)
    # x_1 = s[col]

    ret.update({'x_0': x_0, 'set': s, 'offset': offset, 'set_mask': mask, 'cardinality': cardinality,
                'mean': mean, 'std': std})
    return ret
